...
make_group(gp1, EX(add), EX(or), EX(adc), EX(sbb), EX(and), EX(sub), EX(xor), EX(cmp))
    ...
    /* 0xf6, 0xf7 */
    make_group(gp3, IDEX(src_I, test), EMPTY, EX(not), EX(neg), EX(mul), EX(imul1), EX(div), EX(idiv))
...
opcode_entry opcode_table[512] = {
	...
	/* 0x80 */ IDEXW(I2E, gp1, 1), IDEX(I2E, gp1), IDEXW(I, jcc, 1), IDEX(SI2E, gp1),
	...
	/* 0xb4 */ EMPTY, EMPTY, IDEX(movzx_E2G, movzx), IDEX(movzx_E2G, movsx),
	/* 0xb8 */ EMPTY, EMPTY, EMPTY, EMPTY,
	/* 0xbc */ EMPTY, EMPTY, IDEX(movzx_E2G, movsx), IDEX(movzx_E2G, movzx),
